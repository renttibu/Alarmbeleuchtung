...
class Alarmbeleuchtung extends IPSModule
{
    // Helper
    use AB_alarmLight;
    use AB_alarmProtocol;
    use AB_backupRestore;
    use AB_nightMode;
    ...
    public function Create()
    {
        ...
        // Timers
        $this->RegisterTimer('ActivateAlarmLight', 0, 'AB_ActivateAlarmLight(' . $this->InstanceID . ');');
        $this->RegisterTimer('DeactivateAlarmLight', 0, 'AB_DeactivateAlarmLight(' . $this->InstanceID . ');');
        $this->RegisterTimer('StartNightMode', 0, 'AB_StartNightMode(' . $this->InstanceID . ');');
        $this->RegisterTimer('StopNightMode', 0, 'AB_StopNightMode(' . $this->InstanceID . ',);');
    }
    ...
    public function MessageSink($TimeStamp, $SenderID, $Message, $Data)
    {
        ...
        switch ($Message) {
            ...
            case VM_UPDATE:
                ...
                $scriptText = 'AB_CheckTriggerVariable(' . $this->InstanceID . ', ' . $SenderID . ', ' . $valueChanged . ');';
    ...
    public function GetConfigurationForm()
    {
        ...
        $formData['status'][0] = [
            'code'    => 101,
            'icon'    => 'active',
            'caption' => 'Alarmbeleuchtung wird erstellt',
        ];
        $formData['status'][1] = [
            'code'    => 102,
            'icon'    => 'active',
            'caption' => 'Alarmbeleuchtung ist aktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][2] = [
            'code'    => 103,
            'icon'    => 'active',
            'caption' => 'Alarmbeleuchtung wird gelöscht (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][3] = [
            'code'    => 104,
            'icon'    => 'inactive',
            'caption' => 'Alarmbeleuchtung ist inaktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][4] = [
            'code'    => 200,
            'icon'    => 'inactive',
